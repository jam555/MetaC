...
*/
	...
	typedef libandria4_charresult char_result;
	...
	#define CHAR_RESULT_BODYMATCH( var, onsucc, onfail ) \
		LIBANDRIA4_CHARRESULT_BODYMATCH( var, onsucc, onfail )
	#define CHAR_RESULT_EXPRMATCH( var, onsucc, onfail ) \
		LIBANDRIA4_CHARRESULT_EXPRMATCH( var, onsucc, onfail )
	
	int init();
	void deinit();
	int reinit();
	
		/* I don't think that either of these is actually USABLE outside of */
		/*  source.c, they should probably be removed from the header. */
